...
function Popup({ close, dispatch, index, goals }) {
  ...
  return (
    <motion.div variants={popupContainerAnimation} initial='hidden' animate='visible' exit='exit' className='popup-background'>
      <div ref={popupRef} className='popup'>
        <motion.div className="popup__content" variants={popupFormAnimation}>
          <form className="popup__form" autoComplete="off" 
            onSubmit={(e) => {
              e.preventDefault();
              if (goal._id) dispatch(updateGoal(goal, index))
              else dispatch(newGoal(goal, index))
              close();
            }}>
        ...
        <motion.div className="popup__sidebar" variants={popupFormAnimation}>
